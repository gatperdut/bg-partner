...
export type BuffGroup259Data = BuffGroupData & {
  rightHtml: string;
};

export class BuffGroup259 extends BuffGroup {
  protected buffGroup259Data: BuffGroup259Data;
  ...
  constructor(effs: Eff[]) {
    ...
    const compiled: HandlebarsTemplateDelegate = Handlebars.compile(sheetdata.hbs.buffGroup259);

    const max: number = Math.max(..._.map(effs, (eff: Eff): number => eff.param2));

    const charges: number = effs[0].param1;

    this.buffGroup259Data = {
      ...this.buffGroupData,
    ...
    this.html = compiled(this.buffGroup259Data);
  }
}
