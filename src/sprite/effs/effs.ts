...
export const EffTypes = ['imms', 'hit'] as const;
...
export class Effs {
  ...
  public sources: Record<string, Eff[]> = {};

  public groups: Record<EffType, EffKey[]> = {
    imms: [100, 101, 120, 206, 208, 293, 308, 310, 318, 324],
    hit: [248, 249],
  ...
  public static effsIgnored: EffKey[] = [
    ...
    91, 92, 93, 103, 106, 107, 114, 119, 120, 135, 138, 139, 140, 141, 142, 144, 145, 146, 154, 155,
    156, 158, 169, 170, 173, 174, 177, 184, 186, 187, 206, 208, 215, 233, 240, 265, 267, 271, 275,
    276, 277, 282, 283, 287, 290, 291, 293, 295, 296, 297, 309, 310, 313, 315, 327, 328, 330, 336,
    360, 367,
  ];
  ...
  private runInternal(base: bigint, source: EffSource): void {
    ...
    for (let i: number = 0; i < count; i++) {
      ...
      if (_.includes(this.groups.imms, effKey)) {
        this.effs.imms.push(eff);
      }

      if (_.includes(this.groups.hit, effKey)) {
        this.effs.hit.push(eff);
      }

      if (!_.includes(Effs.effsIgnored, effKey)) {
        if (!this.sources[eff.ressrcCode]) {
          this.sources[eff.ressrcCode] = [];
        }

        this.sources[eff.ressrcCode].push(eff);
      }
    }
  }
}
